...
class TestRemoveLDAPGroupMembers:
    """Tests for remove_ldap_group_members task."""
...
class TestCheckRDFExpiryNotifications:
    ...
    def test_deletion_notification(
        ...
        days_after_expiry = abs(
            settings.RDF_ALLOCATION_DELETION_NOTIFICATION_SCHEDULE[0]
        )
        rdf_allocation.end_date = datetime.now().date() - timedelta(
            days=days_after_expiry
        )
    ...
    def test_multiple_allocations(
        ...
        days_before_expiry = settings.RDF_ALLOCATION_EXPIRY_WARNING_SCHEDULE[
            2
        ]  # 30 days
        rdf_allocation.end_date = datetime.now().date() + timedelta(
            days=days_before_expiry
        ...
        days_after_expiry = abs(
            settings.RDF_ALLOCATION_REMOVAL_WARNING_SCHEDULE[1]
        )  # -3
...
class TestCheckQuotaConsistency:
    """Tests for check_quota_consistency task."""

    # TODO: This could be unparameterized to make it more readable and easy to follow.
